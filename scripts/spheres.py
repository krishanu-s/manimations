...
class SphereStreamLines(StreamLines):
    def __init__(
        self, func, coordinate_system, density=50, sample_coords=None, **kwargs
    ):
...
class TeddyHeadSwirl(InteractiveScene):
    def construct(self):
        ...
        self.play(frame.animate.reorient(-39, 75, 0), run_time=6)
...
class IntroduceVectorField(InteractiveScene):
    def construct(self):
        ...
        self.play(FadeIn(plane), frame.animate.reorient(13, 78, 0), run_time=2)
        self.play(
            frame.animate.reorient(53, 60, 0, (-0.29, 0.09, 0.31), 8.97), run_time=2
        )
        self.wait()
        ...
        def get_tangent_vect():
            ...
            vector = Arrow(
                origin,
                origin + radius * v_func(normalize(origin).reshape(1, -1)).flatten(),
                buff=0,
                thickness=3,
            )
        ...
        self.play(
            Rotate(
                tangent_vect,
                TAU,
                axis=v_tracker.get_center(),
                about_point=tangent_vect.get_start(),
                run_time=2,
            )
        )
        self.wait()
        ...
        v_tracker.add_updater(
            lambda m, dt: m.rotate(60 * DEG * dt, axis=og_vect, about_point=ORIGIN)
        )
        v_tracker.add_updater(
            lambda m, dt: m.rotate(1 * DEG * dt, axis=RIGHT, about_point=ORIGIN)
        )
        ...
        field = self.get_vector_field(
            axes, v_func, 4000, start_point=v_tracker.get_center()
        )
        self.add(field, plane, v_tracker, tangent_vect, v_dot)
        self.play(ShowCreation(field), run_time=5)
        self.wait(4)
        self.play(
            FadeOut(plane),
        ...
        # Show denser field
        dots, dense_dots = [
            DotCloud(fibonacci_sphere(num), radius=0.01) for num in [4000, 400_000]
        ]
        ...
        frame.clear_updaters()
        self.play(
            frame.animate.reorient(-66, 52, 0, (-0.51, 0.27, 0.2), 2.81), run_time=4
        )
        self.wait()
        ...
        stream_lines = AnimatedStreamLines(
            static_stream_lines, lag_range=10, rate_multiple=0.5
        )
        ...
        self.wait(3)
        self.play(
            frame.animate.reorient(129, 70, 0, (-0.18, 0.18, -0.1), 8.22), run_time=20
        )
        self.wait(10)

    def get_vector_field(
        self, axes, v_func, n_points, start_point=None, mvltss=1.0, random_order=False
    ):
    ...
    def old(self):
        ...
        field.add_updater(
            lambda m: m.become(
                self.get_vector_field(int(density_tracker.get_value()), axes, v_func)
            )
        )
        self.add(field)
...
class StereographicProjection(InteractiveScene):
    def construct(self):
        ...
        self.play(
            frame.animate.reorient(-20, 38, 0, (-0.04, -0.03, 0.13), 3.54), run_time=5
        )
        self.wait(5)
        self.play(
        ...
        proj_field.apply_points_function(
            lambda p: inv_streographic_proj(p[:, :2]), about_point=ORIGIN
        )
        ...
        proto_stream_lines.apply_points_function(
            lambda p: inv_streographic_proj(p[:, :2]), about_point=ORIGIN
        )
    ...
    def get_dots_and_lines(
        self, sample_points, color=YELLOW, radius=0.025, stroke_opacity=0.35
    ):
        ...
        proj_lines = VGroup(
            VGroup(Line(OUT, dot.get_center()) for dot in dots)
    ...
    def flow_with_projection_insertion(self):
        ...
        xy_field.apply_depth_test()
        xy_field.set_stroke(opacity=0.0)
        ...
        def update_particles(particles, dt):
            ...
            proj_particles.set_radius(
                (proj_particle_radius_tracker.get_value() * (1.5 - zs)).reshape(-1, 1)
            )
        ...
        moving_particles.add_updater(
            lambda m: m.set_opacity(moving_particles_opacity_tracker.get_value())
        )
...
class SimpleRightwardFlow(InteractiveScene):
    def construct(self):
        ...
        animated_plane_lines = AnimatedStreamLines(
            proto_stream_lines, rate_multiple=0.2
        )
...
class PairsOfNullPoints(InteractiveScene):
    def construct(self):
        ...
        self.play(frame.animate.reorient(-177, 80, 0, (0.0, 0.0, 0.0), 8), run_time=4)
        frame.add_ambient_rotation(1 * DEG)
        ...
        self.play(
            ...
            UpdateFromFunc(
                field,
                lambda m: m.become(
                    self.get_spherical_field(
                        axes,
                        lambda ps: twirl_func(ps, offset_tracker.get_value()),
                        rotation=rotation,
                    )
                ),
            ),
    ...
    def get_streamlines(self, field, axes, radius, density=100):
        streamlines = SphereStreamLines(
            lambda p: field.func(np.array(p).reshape(-1, 3)).flatten(),
            axes,
            density=density,
            solution_time=1.0,
            dt=0.05,
    ...
    def get_spherical_field(
        ...
        field = get_sphereical_vector_field(
            v_func, axes, sample_points, stroke_width=stroke_width
        )
...
class AskAboutOutside(InteractiveScene):
    def construct(self):
        ...
        self.wait()
        self.play(TransformFromCopy(morty, mirror_morty), GrowArrow(in_arrow))
        self.wait()
        ...
        self.wait()
        self.play(
            FadeOut(out_arrows), FadeOut(morty_group), sphere.animate.set_opacity(1)
        )

        # Warp the sphere
        def twist_sphere_uv(u, v):
            return [
                math.cos(u + v - 0.5) * math.sin(v),
                math.sin(u + v - 0.5) * math.sin(v),
                -math.cos(v),
            ]
    ...
    def get_warped_sphere_group(
        self,
        uv_func,
        radius=2,
        mesh_resolution=(61, 31),
        u_range=(0, TAU),
        v_range=(0, PI),
    ):
        surface = ParametricSurface(
            uv_func, u_range=u_range, v_range=v_range, resolution=(201, 101)
        )
...
class InsideOut(InteractiveScene):
    def construct(self):
        ...
        def update_normal_vectors(normal_vectors):
            ...
            du_points = np.array(
                [cap[0].uv_to_point(u + 0.1, v) for u, v in uv_samples]
            )
            dv_points = np.array(
                [cap[0].uv_to_point(u, v + 0.1) for u, v in uv_samples]
            )
            normals = normalize_along_axis(
                np.cross(du_points - points, dv_points - points), 1
            )
        ...
        anti_cap = (
            ...
            .stretch(-1, 2, about_point=ORIGIN)
        )
        ...
        antipode_lines = VGroup(Line(point, -point) for point in pre_points)
    ...
    def old_homotopy(self):
        ...
        self.play(
            ...
            Homotopy(
                lambda x, y, z, t: homotopy(x, y, z, t, -1.0 / inner_scale), cap[1]
            ),
...
class UnitNormals(InteractiveScene):
    def construct(self):
        ...
        uv_samples = np.array(
            [[u, v] for v in np.linspace(0, TAU, 25) for u in np.linspace(0, TAU, 50)]
        )
        ...
        def update_normal_vectors(normal_vectors):
            points = np.array([surface.uv_to_point(u, v) for u, v in uv_samples])
            du_points = np.array(
                [surface.uv_to_point(u + 0.1, v) for u, v in uv_samples]
            )
            dv_points = np.array(
                [surface.uv_to_point(u, v + 0.1) for u, v in uv_samples]
            )
            normals = normalize_along_axis(
                np.cross(du_points - points, dv_points - points), 1
            )
    ...
    def construct(self):
        ...
        lat_label.make_number_changeable("10", edge_to_fix=RIGHT).add_updater(
            lambda m: m.set_value(
                np.round(self.get_lat_lon(*uv_tracker.get_value())[0])
            )
        )
        lon_label.make_number_changeable("10", edge_to_fix=RIGHT).add_updater(
            lambda m: m.set_value(
                np.round(self.get_lat_lon(*uv_tracker.get_value())[1])
            )
        )
        ...
        lon_num_template, lat_num_template = label_template.make_number_changeable(
            "10", replace_all=True
        )
        ...
        self.play(
            frame.animate.reorient(-39, 60, 0, (-0.64, 0.45, 0.06), 4.33), run_time=3
        )
        self.play(
        ...
        self.play(Homotopy(homotopy, group, run_time=10))
        dot.resume_updating()
        self.wait()
        ...
        self.add(lat_line2)
        self.play(
            uv_tracker.animate.increment_value([30 * DEG, 0]).set_anim_args(
                rate_func=wiggle
            ),
            FadeOut(lon_line),
            run_time=3,
        ...
        self.play(
            ...
            .set_anim_args(time_span=(0, 1)),
            uv_tracker.animate.increment_value([0, 30 * DEG]).set_anim_args(
                rate_func=wiggle
            ),
            FadeOut(lat_line),
            run_time=3,
        ...
        # Show normal vector
        normal_vect = Arrow(
            point, point + 0.5 * np.cross(u_step, v_step), thickness=2, buff=0
        )
        ...
        self.add(self.camera.light_source)
        self.play(light.animate.move_to(4 * normalize(light.get_center())), run_time=3)
        ...
        dot.suspend_updating()
        group = Group(
            sphere, mesh, lat_line2, lon_line2, all_labels, tangent_vects, dot
        )
        group.save_state()
        ...
        new_normal_vector = Vector(
            0.35
            * normalize(
                np.cross(tangent_vects[0].get_vector(), tangent_vects[1].get_vector())
            ),
            fill_color=BLUE,
            thickness=1,
        )
        ...
        anti_group = group.copy().scale(
            -1, min_scale_factor=-np.inf, about_point=ORIGIN
        )
        ...
        anti_labels = all_labels.copy().scale(
            -1, min_scale_factor=-np.inf, about_point=ORIGIN
        )
        ...
        self.play(
            TransformFromCopy(all_labels, anti_labels, lag_ratio=0.0002, run_time=3)
        )
        self.wait()
        self.play(
...
class FlowingWater(InteractiveScene):
    def construct(self):
        ...
        water = self.get_water(
            sigma0=0.2, n_droplets=1_000_000, opacity=0.1, refresh_ratio=0.015
        )
        ...
        def get_unit_normal_field(u_range, v_range):
            return get_sphereical_vector_field(
                ...
                ThreeDAxes(),
                points=np.array(
                    [sphere.uv_func(u, v) for u in u_range for v in v_range]
                ),
            )
        ...
        self.play(ShowCreation(sphere), Write(mesh, lag_ratio=1e-3), run_time=2)
        self.wait(2)
        ...
        patch_normals = get_unit_normal_field(
            np.arange(*u_range_params), np.arange(*v_range_params)
        )
        ...
        self.play(Rotate(patch_group, PI, axis=UP), run_time=2)
        self.wait(2.5)
        self.play(
    ...
    def get_water(
        ...
        def flow_out(water, dt):
            ...
            new_points[indices] = np.random.normal(
                0, water.refresh_sigma_tracker.get_value(), (n_refreshes, 3)
            )
            water.set_points(new_points)
            water.set_opacity(
                water.opacity_tracker.get_value() / np.clip(radii, 1, np.inf)
            )
...
class SurfaceFoldedOverSelf(FlowingWater):
    def construct(self):
        ...
        self.play(
            frame.animate.reorient(-25, 76, 0, (-0.13, 0.29, 0.85), 3.38).set_anim_args(
                time_span=(2, 5)
            ),
...
class InsideOutWithNormalField(InteractiveScene):
    def construct(self):
        ...
        outer_normals.add_updater(
            lambda m: m.set_stroke(opacity=float(sign_tracker.get_value() > 0))
        )
        inner_normals.add_updater(
            lambda m: m.set_stroke(opacity=float(sign_tracker.get_value() < 0))
        )

        sphere_group = Group(
            inner_sphere, outer_sphere, mesh, outer_normals, inner_normals
        )
