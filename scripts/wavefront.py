...
class Wavefront:
	...
	def isotopy(self, arc: m.Arc, new_radius: float, **kwargs) -> Isotopy:
		...
		def istpy(x: float, y: float, z: float, t: float, dt: float):
			...
			if dt == 0:
				...
				target_radius = arc.radius * (1 - (t + dt)) + new_radius * (t + dt)
